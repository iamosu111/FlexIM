...
macro_rules! impl_get_info {
    ($name:ident, $func:ident, $param_type:ty) => {
        async fn $name(req: web::Path<($param_type,)>,chain: web::Data<Arc<Mutex<SimChain>>>) -> actix_web::Result<impl Responder> {
            let id = req.into_inner().0;
            let mut chain_ref = chain.lock().unwrap();
            let chain = chain_ref.deref_mut();
            info!("call {} with {:?}", stringify!($func), id);
            let data = chain.$func(id).map_err(handle_err)?;
...
// 使用宏时，指定相应的参数类型
impl_get_info!(web_get_blk_header, read_block_header, IdType);
impl_get_info!(web_get_blk_data, read_block_data, IdType);
impl_get_info!(web_get_inter_index, read_inter_index, IdType);
impl_get_info!(web_get_transaction, read_transaction, IdType);
impl_get_info!(web_get_intra_index, read_intra_index, IdType);
impl_get_info!(web_get_index_config, read_index_config, KeyType); 





async fn web_get_param(chain: web::Data<Arc<Mutex<SimChain>>>) -> actix_web::Result<impl Responder> {
    info!("call get_parameter");
    let mut chain_ref = chain.lock().unwrap();
    let chain = chain_ref.deref_mut();
    let data = chain.get_parameter().map_err(handle_err)?;
    Ok(HttpResponse::Ok().json(data))
}

async fn web_get_inter_indexs(chain: web::Data<Arc<Mutex<SimChain>>>) -> actix_web::Result<impl Responder> {
    info!("call get_inter_indexs");
    let mut chain_ref = chain.lock().unwrap();
    let chain = chain_ref.deref_mut();
    let data = chain.read_inter_indexs().map_err(handle_err)?;
    Ok(HttpResponse::Ok().json(data))
}

async fn web_query(query_param: web::Json<QueryParam>,chain: web::Data<Arc<Mutex<SimChain>>>) -> actix_web::Result<impl Responder>{
    info!("into web_query");
    let mut chain_ref = chain.lock().unwrap();
    let chain = chain_ref.deref_mut();
    let result = historical_query(&query_param, chain).map_err(handle_err)?;
...
async fn main() -> actix_web::Result<()> {
    ...
    let chain_arc=Arc::new(Mutex::new(SimChain::open(&opts.db_path).map_err(handle_err)?));
    HttpServer::new(move|| {
        App::new()
            .app_data(web::Data::new(chain_arc.clone()))
            .wrap(
                Cors::default()
                    .send_wildcard()
            ...
            .route("/get/intra_index/{id}", web::get().to(web_get_intra_index))
