...
use super::{utils::{Hashable,HashUtils}, BLOOMFILTER_CAPASITY, BLOOMFILTER_FP};
...
impl<T> Tree<T> {
    ...
    // Compute union of bloomfilter
    pub fn union_bloom_filter(left: &Tree<T>, right: &Tree<T>) -> SeededBloomFilter{
        let mut new_bloom_filter = SeededBloomFilter::new(BLOOMFILTER_CAPASITY, BLOOMFILTER_FP);
        match left {
            Tree::Empty { ref hash } => {new_bloom_filter.insert(hash)},
            Tree::Leaf { ref hash, ..} => {new_bloom_filter.insert(hash)},
            Tree::Node { ref bloom_filter, .. } => {new_bloom_filter = new_bloom_filter.union(bloom_filter)},
        };
        match right {
            Tree::Empty { ref hash } => {new_bloom_filter.insert(hash)},
            Tree::Leaf { ref hash, ..} => {new_bloom_filter.insert(hash)},
